...
@section('content')
    @php
        ...
        $anhChinh = count($hinhAnhs) > 0 ? asset($hinhAnhs[0]) : asset('img/no-image.png');
        ...
        <div class="container py-5">
            <div class="row g-4">
                ...
                <div class="col-lg-5 col-xl-3 wow fadeInUp" data-wow-delay="0.1s">
                    ...
                    <div class="small text-muted mb-1 mt-3">Trạng thái</div>
                    <div class="fw-bold mb-3">
                        <span class="badge bg-{{ $sanPham->trangThaiText == 'Còn hàng' ? 'success' : 'secondary' }}">
                            {{ $sanPham->trangThaiText }}
                        </span>
                    </div>

                    <div class="small text-muted mb-1">Thống kê</div>
                    <div class="d-flex gap-3 small">
                        <span>Lượt xem: <strong id="viewCount">{{ (int) ($thongKe->LuotXem ?? 0) }}</strong></span>
                        <span>Yêu thích: <strong id="favCount">{{ (int) ($thongKe->LuotYeuThich ?? 0) }}</strong></span>
                    </div>
                ...
                <div class="col-lg-7 col-xl-9 wow fadeInUp" data-wow-delay="0.1s">
                    <div class="row g-4 single-product">
                        ...
                        <div class="col-xl-6">
                            ...
                            <p class="mb-2">Danh mục: <span class="fw-bold">{{ $sanPham->tenDanhMuc ?? '-' }}</span></p>
                            ...
                            <div class="d-flex flex-column mb-3">
                                <small>Mã sản phẩm: SP{{ $sanPham->MaSP }}</small>
                                <small>Còn hàng: <strong id="tonKho" class="text-primary">--</strong></small>
                            </div>

                            <p class="mb-4" style="white-space:pre-line">
                                {{ $sanPham->MoTa ?? 'Chưa có mô tả' }}
                            </p>
                            ...
                            <div class="input-group quantity mb-4" style="width: 130px;">
                                ...
                                <input id="qtyInput" type="text"
                                    class="form-control form-control-sm text-center border-0 qty-input" value="1">
                            ...
                            <form method="POST" action="{{ route('cart.add') }}" class="mb-4"
                                ...
                                <button class="btn btn-outline-danger rounded-pill px-4 py-2 ms-2" type="button"
                                    onclick="yeuThich({{ $sanPham->MaSP }}, this)">
                                        ...
                                        <div class="bg-light rounded p-4">
                                            ...
                                            @if (session()->has('khachhang'))
                                                <form method="POST"
                                                    ...
                                                    <button class="btn btn-primary mt-3" type="submit">Gửi đánh
                                                        giá</button>
                                                </form>
                                            @else
                                                <div class="alert alert-warning">
                                                    Bạn cần <a href="/dang-nhap">đăng nhập</a> để đánh giá sản phẩm.
                                                </div>
                                            @endif
                                            {{-- end phat --}}
                                        </div>
                            ...
                            <div class="row g-4">
                                @if (($related->count() ?? 0) == 0)
                                    <div class="col-12">
                                        <div class="alert alert-info mb-0">Chưa có sản phẩm liên quan.</div>
                                    </div>
                                @else
                                            ...
                                            <div class="border rounded p-3 h-100 bg-light">
                                                <a href="{{ route('shop.show', $r->slug) }}"
                                                    ...
                                                    <div class="text-primary fw-bold mt-1">
                                                        {{ number_format((float) ($r->giaMin ?? 0), 0, ',', '.') }}
                                                        d
                                                    </div>
                                                </a>
                                            </div>
        ...
        @push('scripts')
            <script>
                ...
                function beforeAddCart() {
                    ...
                    if (!maBT) {
                        alert('Vui lòng chọn kích thước (size) trước khi thêm giỏ hàng');
                        return false;
                    }
                ...
                document.querySelectorAll('.size-btn').forEach(btn => {
                    btn.addEventListener('click', () => {
                        ...
                        document.getElementById('tonKho').innerText = ton + ' sản phẩm';
                    });
                });
    ...
    @push('scripts')
        <script>
            async function yeuThich(maSP, btn) {
                try {
                    const res = await fetch("{{ route('yeuthich.toggle') }}", {
                        method: "POST",
                    ...
                    if (res.status === 401) {
                        alert("Ban can dang nhap de dung chuc nang yeu thich");
                        return;
                    }
                    ...
                    // cap nhat thong ke (neu ban co API thong-ke)
                    await refreshThongKe();
                } catch (e) {
                    console.log(e);
                    alert("Co loi khi yeu thich");
                }
            }
            ...
            async function refreshThongKe() {
                try {
                    ...
                    const data = await res.json();
                    if (document.getElementById('viewCount')) document.getElementById('viewCount').innerText = data.luotXem;
                    if (document.getElementById('favCount')) document.getElementById('favCount').innerText = data
                        .luotYeuThich;
                } catch (e) {
                    console.log(e);
                }
